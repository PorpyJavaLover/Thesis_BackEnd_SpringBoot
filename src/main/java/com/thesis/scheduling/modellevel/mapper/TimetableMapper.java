...
public class TimetableMapper {
	...
	public Iterable<M_Timetable_ShowTimeRemain_Response> toMTimeStartStaff(Iterable<Timetable> source , Collection<NotTeach> sourceC , String sourceB) {
		...
		Collection<M_Timetable_ShowTimeRemain_Response> targetF = relativeComplementBInABussy(targetB, targetA , targetE);

		Integer previous = deconvertStartTime(sourceB);

		if (previous != 0) {

			Collections.reverse((List<M_Timetable_ShowTimeRemain_Response>) targetF);
			Collection<M_Timetable_ShowTimeRemain_Response> targetD = new ArrayList<M_Timetable_ShowTimeRemain_Response>();
			
			for (M_Timetable_ShowTimeRemain_Response targetSubC : targetF) {
				if (targetSubC.getValue().compareTo(sourceB) < 0
						&& (deconvertStartTime(targetSubC.getValue()) - previous) == -1) {

					targetD.add(targetSubC);
					previous = deconvertStartTime(targetSubC.getValue());
				}

			}
			
			Collections.reverse((List<M_Timetable_ShowTimeRemain_Response>) targetD);
	...
	public Iterable<M_Timetable_ShowTimeRemain_Response> toMTimeEndStaff(Iterable<Timetable> source , Collection<NotTeach> sourceC , String sourceB) {
	...
	public Collection<M_Timetable_ShowTimeRemain_Response> relativeComplementBInABussy(
			Collection<M_Timetable_ShowTimeRemain_Response> cA, Collection<M_Timetable_ShowTimeRemain_Response> cB , Collection<M_Timetable_ShowTimeRemain_Response> cC) {
